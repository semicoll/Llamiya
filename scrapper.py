...
import json
import re
import sys  # Add this import for the pip installation fix
def extract_range_with_selenium(driver, operator_name):
    range_data = {}ith_selenium(driver, operator_name):
    try:e_data = {}
        base_url = "https://arknights.fandom.com/wiki/"
        url = f"{base_url}{operator_name.replace(' ', '_')}"
        driver.get(url)rl}{operator_name.replace(' ', '_')}"
        ...
        for i, level in enumerate(levels):e 2"]
            try:evel in enumerate(levels):
                column_xpath = f'{range_table_xpath}/tbody/tr[2]/td[{i + 1}]'
                column = driver.find_element(By.XPATH, column_xpath){i + 1}]'
                ...
                for idx, cell in enumerate(grid_cells):
                    ...
                    row.append(symbol)ackground: #27A6F3' in cell.get_attribute("style") else '□'
                    row.append(symbol)
                    if (idx + 1) % 5 == 0:
                        range_representation.append(row)
                        row = []presentation.append(row)
                        row = []
                if row:
                    range_representation.append(row)
                    range_representation.append(row)
                ...
                range_data[level] = "N/A"range for {level}: {e}")
...
def operator_info(driver, operator_name):
    ...
    try:
        ...
        with open(output_path, "w", encoding="utf-8") as json_file:on")
...
def extract_and_write_stats_with_selenium(driver, operator_name):
    stats_data = {}te_stats_with_selenium(driver, operator_name):
    try:s_data = {}
        ...
        rows = stats_table.find_elements(By.XPATH, ".//tr")able_xpath)
        ...
        for row in rows[1:]:
            ...
            except Exception as e:te_name] = {level: stats[i] if i < len(stats) else "" for i, level in enumerate(elite_levels)}
        ...
        if os.path.exists(file_path):
            with open(file_path, "r", encoding="utf-8") as json_file:
                operator_data = json.load(json_file)8") as json_file:
        else:   operator_data = json.load(json_file)
            operator_data = {}
            operator_data = {}
...
def extract_potential_with_selenium(driver, operator_name):
    potential_data = {}ith_selenium(driver, operator_name):
    try:ntial_data = {}
        ...
        rows = potential_table.find_elements(By.XPATH, ".//tr")al_table_xpath)
        ...
        for row in rows[1:]:
            try:in rows[1:]:
                if "OR" in row.text and "Potential" not in row.text:
                    continueow.text and "Potential" not in row.text:
                    continue
                potential_effect = row.find_element(By.XPATH, ".//td[2]").text.strip()
                potential_key = f"Pot {pot_counter}"By.XPATH, ".//td[2]").text.strip()
...
def extract_promotion_with_selenium(driver, operator_name):
    promotion_data = {}ith_selenium(driver, operator_name):
    try:otion_data = {}
        ...
        for level, xpaths in promotion_rows.items():
            try:l, xpaths in promotion_rows.items():
                toggle_button = driver.find_element(By.XPATH, xpaths["button"])
                driver.execute_script("arguments[0].click();", toggle_button)])
                time.sleep(1)e_script("arguments[0].click();", toggle_button)
                time.sleep(1)
                content_div = driver.find_element(By.XPATH, xpaths["content"])
                details = content_div.text.strip()By.XPATH, xpaths["content"])
                details = details.replace("\\n", "\n")
                details = details.replace("\\n", "\n")
                material_images = content_div.find_elements(By.XPATH, ".//img")
                text_items = details.split()v.find_elements(By.XPATH, ".//img")
                quantities = []tails.split()
                quantities = []
                for item in text_items:
                    item = item.strip()
                    if item.isdigit() or (item[:-1].isdigit() and item[-1] == 'K'):
                        quantities.append(item):-1].isdigit() and item[-1] == 'K'):
                ...
                for quantity in quantities:
                    if quantity.endswith('K') and lmd_quantity is None:
                        lmd_quantity = quantitynd lmd_quantity is None:
                    else:md_quantity = quantity
                ...
                for img in material_images:
                    ...
                    if not img_name or "icon" in img_name.lower():
                        continueame or "icon" in img_name.lower():
                        continue
                    if img_name.lower().endswith(('.png', '.jpg', '.jpeg')):
                        img_name = img_name.rsplit('.', 1)[0]pg', '.jpeg')):
                ...
                if valid_images and lmd_quantity:
                    ...
                    for i, img_name in enumerate(valid_images[1:]):
                        if i < len(other_quantities):d_images[1:]):
                            materials.append({"name": img_name, "quantity": other_quantities[i]})
                        else:aterials.append({"name": img_name, "quantity": other_quantities[i]})
                            materials.append({"name": img_name, "quantity": "1"})
                else:       materials.append({"name": img_name, "quantity": "1"})
                    for i, img_name in enumerate(valid_images):
                        if i < len(quantities):e(valid_images):
                            materials.append({"name": img_name, "quantity": quantities[i]})
                        else:aterials.append({"name": img_name, "quantity": quantities[i]})
                            materials.append({"name": img_name, "quantity": "1"})
                            materials.append({"name": img_name, "quantity": "1"})
...
def extract_skills_with_selenium(driver, operator_name):
    skills_data = []ith_selenium(driver, operator_name):
    skills_data = []
    try:
        ...
        for skill_index, div in enumerate(skill_divs, 2):
            try:l_index, div in enumerate(skill_divs, 2):
                try:
                    skill_name_elem = div.find_element(By.XPATH, './/b[@style="font-size:14px;"]')
                    skill_name = skill_name_elem.text.strip()TH, './/b[@style="font-size:14px;"]')
                    if not skill_name:_name_elem.text.strip()
                        continue_name:
                    print(f"Processing skill: {skill_name}")
                except:nt(f"Processing skill: {skill_name}")
                    continue
                    continue
                try:
                    skill_image = div.find_element(By.XPATH, './/img').get_attribute('src')
                except:ll_image = div.find_element(By.XPATH, './/img').get_attribute('src')
                    skill_image = ""
                    skill_image = ""
                types = []
                try:s = []
                    type_container = div.find_element(By.XPATH, './table/tbody/tr/td[2]/div')
                    type_divs = type_container.find_elements(By.XPATH, './div')tr/td[2]/div')
                    type_divs = type_container.find_elements(By.XPATH, './div')
                    for type_div in type_divs:
                        type_text = type_div.text.strip()
                        if type_text:ype_div.text.strip()
                            types.append(type_text)
                except Exception as e:nd(type_text)
                    print(f"Error extracting skill types: {e}")
                    print(f"Error extracting skill types: {e}")
                try:
                    toggle_button = div.find_element(By.XPATH, './button/span')
                    toggle_text = toggle_button.text.strip()H, './button/span')
                ...
                try:
                    ...
                    for row_index, row in enumerate(level_rows):
                        try:index, row in enumerate(level_rows):
                            if row_index < 7:
                                level = str(row_index + 1)
                            else:evel = str(row_index + 1)
                            ...
                            sp_cost = row.find_element(By.XPATH, './td[3]').text.strip()p()
                            ...
                            level_data = {
                                ...
                                "sp_cost": sp_cost,al_sp,
                                "duration": duration
                            }   "duration": duration
                            levels.append(level_data)
                        except Exception as e:l_data)
                            print(f"Error extracting row {row_index+1}: {e}")
                except Exception as e:ror extracting row {row_index+1}: {e}")
        ...
        else:rint(f"Saved {len(skills_data)} skills for {operator_name}")
...
def extract_skill_upgrade_costs(driver, operator_name):    
    upgrade_costs = {}ade_costs(driver, operator_name):    
    mastery_costs = {}
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if "Show" in toggle_button.text:ection.find_element(By.XPATH, './/button/span')
                    driver.execute_script("arguments[0].click();", toggle_button)
                    time.sleep(1)e_script("arguments[0].click();", toggle_button)
            except: time.sleep(1)
                pass
            ...
            for level in range(2, 8):
                try:l in range(2, 8):
                    ...
                    level_row = upgrade_table.find_element(By.XPATH, level_xpath)img[contains(@data-image-name, "Rank_{level}")]]'
                    level_row = upgrade_table.find_element(By.XPATH, level_xpath)
                    materials_cell = level_row.find_element(By.XPATH, './td')
                    materials = [] = level_row.find_element(By.XPATH, './td')
                    materials = []
                    ...
                    for div in material_divs:
                        try:in material_divs:
                            ...
                            quantity = quantity_div.text.strip()PATH, './/div[contains(@style, "position:absolute; right:0px; bottom:0px")]/div')
            ...
            mastery_rows = upgrade_table.find_elements(By.XPATH, 
                ...
                'contains(@data-image-name, "Rank_9") or ' +) or ' +
            ...
            if mastery_rows:
                ...
                material_index = 0ery = len(all_mastery_materials) // 9
                skills = ["Skill1", "Skill2", "Skill3"]
                mastery_levels = ["M1", "M2", "M3"]l3"]
                ...
                for skill in skills:
                    for mastery in mastery_levels:
                        ...
                        materials_for_this_mastery = all_mastery_materials[material_index:end_index]rials))
                        mastery_costs[skill]["masteries"][mastery] = materials_for_this_masteryndex]
                        material_index = end_indexeries"][mastery] = materials_for_this_mastery
                        material_index = end_index
            if upgrade_costs or any(skill["masteries"] for skill in mastery_costs.values()):
                output_dir = "operator"ll["masteries"] for skill in mastery_costs.values()):
                os.makedirs(output_dir, exist_ok=True)
                file_path = os.path.join(output_dir, f"{operator_name}.json")
                file_path = os.path.join(output_dir, f"{operator_name}.json")
                if os.path.exists(file_path):
                ...
                for skill_key, skill_data in mastery_costs.items():
                    if any(skill_data["masteries"].values()):ems():
...
def extract_talents_with_selenium(driver, operator_name):
    talents_data = []ith_selenium(driver, operator_name):
    talents_data = []
    try:
                ...
                for row_index, row in enumerate(rows):
                    try:index, row in enumerate(rows):
                        ...
                        try:
                            ...
                            for img in img_elements:l.find_elements(By.XPATH, './/img')
                                ...
                                if ("potential" in img_src.lower() or 
                                    ...
                                    is_potential = Truetitle.lower()):
                                    potential_text = img_title or img_alt or "Potential Upgrade"
                                    breaktial_text = img_title or img_alt or "Potential Upgrade"
                        except:     break
                            pass
                            pass
                        if is_potential:
        ...
        if talents_data:
            ...
            file_path = os.path.join(output_dir, f"{operator_name}.json")
            if os.path.exists(file_path):
                with open(file_path, "r", encoding="utf-8") as json_file:  # Fixed typo: "file_file" to "file_path"
                    operator_data = json.load(json_file)8") as json_file:
            else:   operator_data = json.load(json_file)
                operator_data = {}
                operator_data = {}
        ...
        else:rint(f"Saved {len(talents_data)} talents for {operator_name}")
...
if __name__ == "__main__":
    try:e__ == "__main__":
        ...
        if not chrome_check.stdout:un(['which', 'google-chrome'], capture_output=True, text=True)
            print("Chrome not found. Installing Chrome dependencies for WSL...")
            %pip install --upgrade pip webdriver-manager seleniumes for WSL...")manager', 'selenium'])
    ...
    except Exception as e: need to install Chrome in WSL with: sudo apt update && sudo apt install -y wget unzip fonts-liberation libasound2 libatk-bridge2.0-0 libatk1.0-0 libatspi2.0-0 libcairo2 libcups2 libcurl3-gnutls libdrm2 libgbm1 libgtk-3-0 libnspr4 libnss3 libpango-1.0-0 libxkbcommon0 libxcomposite1 libxdamage1 libxfixes3 libxrandr2 xdg-utils && wget https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb && sudo dpkg -i google-chrome-stable_current_amd64.deb && sudo apt-get -f install") need to install Chrome in WSL with: sudo apt update && sudo apt install -y wget unzip fonts-liberation libasound2 libatk-bridge2.0-0 libatk1.0-0 libatspi2.0-0 libcairo2 libcups2 libcurl3-gnutls libdrm2 libgbm1 libgtk-3-0 libnspr4 libnss3 libpango-1.0-0 libxkbcommon0 libxcomposite1 libxdamage1 libxfixes3 libxrandr2 xdg-utils && wget https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb && sudo dpkg -i google-chrome-stable_current_amd64.deb && sudo apt-get -f install")
        print(f"Error checking Chrome installation: {e}")
        print(f"Error checking Chrome installation: {e}")        print(f"Error checking Chrome installation: {e}")
    os.environ['WDM_LOG_LEVEL'] = '0'
    os.environ['WDM_PROGRESS_BAR'] = '0'
    os.environ['WDM_PROGRESS_BAR'] = '0'    os.environ['WDM_PROGRESS_BAR'] = '0'
    chrome_options = Options()
    chrome_options.add_argument("--headless")
    chrome_options.add_argument("--disable-gpu")
    chrome_options.add_argument("--window-size=1920x1080")
    chrome_options.add_argument("--no-sandbox")1920x1080")1920x1080")
    ...
    try:
        ...
        chrome_version = chrome_version_cmd.stdout.strip()e', '--version'], capture_output=True, text=True)e', '--version'], capture_output=True, text=True)
        print(f"Detected Chrome version: {chrome_version}")
    except:nt(f"Detected Chrome version: {chrome_version}")nt(f"Detected Chrome version: {chrome_version}")
        print("Could not detect Chrome version. Make sure Chrome is installed.")
        print("Could not detect Chrome version. Make sure Chrome is installed.")        print("Could not detect Chrome version. Make sure Chrome is installed.")
    # Initialize the central WebDriver
    driver = Nonethe central WebDriverthe central WebDriver
    try:er = Noneer = None
        driver_path = ChromeDriverManager().install()
        print(f"Driver installed at: {driver_path}")))
        print(f"Driver installed at: {driver_path}")print(f"Driver installed at: {driver_path}")
        os.chmod(driver_path, 0o755)
        os.chmod(driver_path, 0o755)os.chmod(driver_path, 0o755)
        driver = webdriver.Chrome(service=Service(driver_path), options=chrome_options)
        driver = webdriver.Chrome(service=Service(driver_path), options=chrome_options)driver = webdriver.Chrome(service=Service(driver_path), options=chrome_options)
        # Scrape the list of 6-star operators
        url = "https://arknights.fandom.com/wiki/Operator/6-star"
        driver.get(url)arknights.fandom.com/wiki/Operator/6-star"arknights.fandom.com/wiki/Operator/6-star"
        time.sleep(3)l)l)
        ...
        operator_elements = driver.find_elements(By.XPATH, '/html/body/div[4]/div[4]/div[2]/main/div[3]/div/div[1]/div/div/table/tbody/tr/td[2]/a')operator_elements = driver.find_elements(By.XPATH, '/html/body/div[4]/div[4]/div[2]/main/div[3]/div/div[1]/div/div/table/tbody/tr/td[2]/a')
        operator_names = [element.text for element in operator_elements if element.text]
        operator_names = [element.text for element in operator_elements if element.text]operator_names = [element.text for element in operator_elements if element.text]
        print("6-Star Operators:")
        for name in operator_names:
            print(name)rator_names:rator_names:
            print(name)print(name)
        print(f"\nTotal 6-star operators found: {len(operator_names)}")
        print(f"\nTotal 6-star operators found: {len(operator_names)}")print(f"\nTotal 6-star operators found: {len(operator_names)}")
        # Example of processing one operator with the central driver
        if operator_names:ssing one operator with the central driverssing one operator with the central driver
            test_operator = operator_names[0]
            print(f"\nProcessing example operator: {test_operator}")
            print(f"\nProcessing example operator: {test_operator}")print(f"\nProcessing example operator: {test_operator}")    
            # Extract data using the central driver
            operator_info(driver, test_operator)verver
            extract_and_write_stats_with_selenium(driver, test_operator)
            extract_potential_with_selenium(driver, test_operator)rator)rator)operator_name)
            extract_promotion_with_selenium(driver, test_operator)or_name)
            extract_skills_with_selenium(driver, test_operator)or)or)rator_name)
            extract_skill_upgrade_costs(driver, test_operator)))tor_name)
            extract_talents_with_selenium(driver, test_operator)e)
            extract_talents_with_selenium(driver, test_operator)extract_talents_with_selenium(driver, test_operator)        extract_talents_with_selenium(driver, operator_name)
            print(f"Completed processing {test_operator}")
            print(f"Completed processing {test_operator}")    print(f"Completed processing {test_operator}")            print(f"Completed processing {operator_name}")
    except Exception as e:
        print(f"Error initializing WebDriver: {e}")
        print("If you're using WSL, you may need to install Chrome in the WSL environment")
    finally:t("If you're using WSL, you may need to install Chrome in the WSL environment")t("If you're using WSL, you may need to install Chrome in the WSL environment")
        if driver:
            driver.quit()river: {e}")
            driver.quit()            driver.quit()        print("If you're using WSL, you may need to install Chrome in the WSL environment")
    finally:
